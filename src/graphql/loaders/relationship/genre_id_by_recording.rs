...
impl Loader<PageKey> for GenreIdsByRecordingLoader {
    type Value = Vec<i32>;
    type Error = async_graphql::Error;

    async fn load(&self, keys: &[PageKey]) -> Result<HashMap<PageKey, Self::Value>, Self::Error> {
        info!(count = keys.len(), "GenreIdsByRecordingLoader batch load");
        ...
        for ((after, first), entity_ids) in groups {
            let rows = sqlx::query_as!(
                RecordingGenreIdRow,
                r#"SELECT recording AS "recording!", id AS "id!"
                FROM (
            ...
            for row in rows {
                group_result.entry(row.recording).or_default().push(row.id);
            }
