...
impl Loader<PageKey> for GenreIdsByArtistLoader {
    type Value = Vec<i32>;
    type Error = async_graphql::Error;

    async fn load(&self, keys: &[PageKey]) -> Result<HashMap<PageKey, Self::Value>, Self::Error> {
        info!(count = keys.len(), "GenreIdsByArtistLoader batch load");
        ...
        for ((after, first), entity_ids) in groups {
            let rows = sqlx::query_as!(
                ArtistGenreIdRow,
                r#"SELECT artist AS "artist!", id AS "id!"
                FROM (
            ...
            for row in rows {
                group_result.entry(row.artist).or_default().push(row.id);
            }
