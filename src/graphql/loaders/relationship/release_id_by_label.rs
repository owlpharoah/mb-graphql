...
impl Loader<PageKey> for ReleaseIdsByLabelLoader {
    type Value = Vec<i32>;
    type Error = async_graphql::Error;

    async fn load(&self, keys: &[PageKey]) -> Result<HashMap<PageKey, Self::Value>, Self::Error> {
        info!(count = keys.len(), "ReleaseIdsByLabelLoader batch load");
        ...
        for ((after, first), entity_ids) in groups {
            let rows = sqlx::query_as!(
                LabelReleaseIdRow,
                r#"SELECT label AS "label!", release AS "release!"
                FROM (
            ...
            for row in rows {
                group_result.entry(row.label).or_default().push(row.release);
            }
