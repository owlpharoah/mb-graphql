...
pub struct IsoCode1ByAreaLoader {
    pub pool: PgPool,
}

impl Loader<i32> for IsoCode1ByAreaLoader {
    type Value = Vec<String>;
    type Error = async_graphql::Error;

    async fn load(&self, area_ids: &[i32]) -> Result<HashMap<i32, Self::Value>, Self::Error> {
        info!(count = area_ids.len(), "IsoCode1ByAreaLoader batch load");
        ...
        info!(rows = rows.len(), "IsoCode1ByAreaLoader query returned");
